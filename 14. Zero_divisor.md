...
$$\{\text{zero divisors in } \mathbb{Z}_n\} = \{\bar{k} \in \mathbb{Z}_n \mid \bar{k}\bar{x} = \bar{0} \text{ has a non-zero solution}\}$$
...
$\{\text{left zero\ divisor\ in\ }M_n(\mathbb{R})\} = \{A \in M_n(\mathbb{R}) \mid \exists\ B\neq 0\ \text{s.t.}\ AB=0\}$
  $= \{A \in M_n(\mathbb{R}) \mid A\ \text{is singular}\}$
